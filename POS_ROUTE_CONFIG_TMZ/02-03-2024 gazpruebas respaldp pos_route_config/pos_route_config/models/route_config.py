...
class RC_report(models.Model): 
    _name = 'pos_route_config.report_create'
    _description = 'reportes'

    fecha_filtro = fields.Date(string='Fecha')

    relacion_ids = fields.Many2many(comodel_name='pos_route_config.data_storage_report',  inverse_name='data_ids', string='Relacion')
    ...
    def _get_name(self): 
        for doc in self:
            if doc.fecha_filtro:
                rec = self.env['pos_route_config.route_config'].search([('anno', '>=', doc.fecha_filtro), ('anno', '<=', doc.fecha_filtro), ("company_id.name", "=", doc.company_id.name)]) #search([('anno', '=', doc.fecha_filtro)]) 
                self.relacion_ids = [(5,0,0)]
                for res in rec:
                    line = {}
                    line['Fecha de creación'] = res.date
                    line['Referencia de ruta'] = res.name
                    line['Punto de venta'] = res.pos_config_id
                    line['Litrometro inicial'] = res.st_volume_meter
                    line['Litrometro final'] = res.en_volume_meter
                    line['Dif. Litrometro'] = res.dif_volume_meter
                    line['Peso inicial'] = res.st_weight
                    line['Peso final'] = res.en_weight_aux
                    line['Dif. Peso'] = res.dif_weight
                    line['Porcentaje inicial'] = res.st_percent
                    line['Porcentaje final'] = res.en_percent
                    line['Dif. Porcentaje'] = res.dif_percent
                    line['Promedio sector'] = res.avg_sector
                    line['Promedio general'] = res.hg_avg
                    line['Carburacion inicial'] = res.st_carburation
                    line['Carburacion final'] = res.en_carburation
                    line['Dif. Carburacion'] = res.dif_carburation
                    line['Rotagauge inicial'] = res.st_gauge
                    line['Rotagauge final'] = res.en_gauge
                    line['Litros por cobrar'] = res.liters_receivable
                    line['Litros para recargar'] = res.refuel_liters
                    line['Estatus'] = res.state
                    line['Empleado'] = res.employee_ids
                    line['Ayudante'] = res.x_studio_ayudante.id
                    line['Justificacion'] = res.justification
                    self.relacion_ids = [(0,0,line)]



class reporte_s(models.Model): #esta almacena los datos
    _name = 'pos_route_config.data_storage_report'
    _description = 'Almacen de datos'

    data_ids = fields.Many2one(comodel_name='pos_route_config.report_create', string='Variables', readonly=True)# 
    ...
    #Litros por cobrar = liters_receivable
    refuel_liters = fields.Float(string="Liters to refuel")
    state = fields.Selection(string='Status', default='new',
        selection=[('new', 'New'), 
                   ('start', 'Route Configuration'), 
