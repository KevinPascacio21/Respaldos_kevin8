...
class RouteConfiguration(models.Model):
    ...
    #fecha_filtro = fields.Date(string='Fecha de creación del reporte: ', default=fields.Date.today()) #AGREGADO K
...
class RC_report(models.Model): 
    ...
    company_id = fields.Many2one(comodel_name='res.company', required=True, default=lambda self: self.env.company)
    fecha_filtro = fields.Date(string='Fecha de creación del reporte: ', default=fields.Date.today())
        ...
        for doc in self:
            if doc.fecha_filtro:
                ...
                for res in rec:
                    line = {}
                    line['date'] = res.date
                    #line['fecha_filtro'] = res.fecha_filtro
                    line['name'] = res.name
...
class reporte_s(models.Model): #esta almacena los datos
    ...
    #Datatime fecha y hora de creacion
