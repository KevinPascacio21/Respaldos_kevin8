...
class RouteConfigReport(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, route_config):
        for ctrl in route_config:
            ...
            format_cell_orange = workbook.add_format({'font_color': 'black', 'bg_color': '#eea583', 'border': True, 'border_color': '#F1FADA', 'align': 'center'})
            format_cell_grey = workbook.add_format({'font_color': 'black', 'bg_color': '#cacaca', 'border': True, 'border_color': '#F1FADA', 'align': 'center'})
            format_cell_blue = workbook.add_format({'font_color': 'black', 'bg_color': '#b2e2fc', 'border': True, 'border_color': '#F1FADA', 'align': 'center'})
            format_cell_yellow = workbook.add_format({'font_color': 'black', 'bg_color': '#eaf293', 'border': True, 'border_color': '#F1FADA', 'align': 'center'})
            format_cell_green = workbook.add_format({'font_color': 'black', 'bg_color': '#c2ffb0', 'border': True, 'border_color': '#F1FADA', 'align': 'center'})
            ...
            for line in ctrl.relacion_ids:
              #   sheet.write(row, 2, line.route_creation_date , format_cell_Date) dejarlo comentado
                route_creation_datetime = fields.Datetime.from_string(line.route_creation_date)#
                new_datetime = route_creation_datetime - timedelta(hours=7)#
                sheet.write(row, 2, new_datetime, format_cell_Date)#

              #   sheet.write(row, 3, line.departure_date , format_cell_Date) dejarlo comentado
                departure_datetime = fields.Datetime.from_string(line.departure_date)#
                new_departure_datetime = departure_datetime - timedelta(hours=7)#
                sheet.write(row, 3, new_departure_datetime, format_cell_Date)#

              #   sheet.write(row, 4, line.arrival_date , format_cell_Date) dejarlo comentado
                arrival_datetime = fields.Datetime.from_string(line.arrival_date)
              #   new_arrival_datetime = arrival_datetime - timedelta(hours=7)#
              #   sheet.write(row, 4, new_arrival_datetime, format_cell_Date)#
                if arrival_datetime:
                     new_arrival_datetime = arrival_datetime - timedelta(hours=7)
                else:
                     new_arrival_datetime = None  # or some other default value
                sheet.write(row, 4, new_arrival_datetime, format_cell_Date)#
                ...
                if line.state == "new": 
                       ...
                       sheet.write(row, 25, "Nuevo", format_cell_white)
                        ...
                        sheet.write(row, 25, "Configuración de ruta", format_cell_yellow)
                elif line.state == "Route Configuration":
                       sheet.write(row, 25, "Configuración de ruta", format_cell_yellow)
                elif line.state == "sale":
                       sheet.write(row, 25, "Ventas", format_cell_orange)
                elif line.state == "Sales":
                       sheet.write(row, 25, "Ventas", format_cell_orange)
                elif line.state == "reception":
                       sheet.write(row, 25, "Recepción de rutas", format_cell_blue)
                elif line.state == "Route Reception":
                       sheet.write(row, 25, "Recepción de rutas", format_cell_blue)
                elif line.state == "concil":
                       sheet.write(row, 25, "Conciliación de venta", format_cell_grey)
