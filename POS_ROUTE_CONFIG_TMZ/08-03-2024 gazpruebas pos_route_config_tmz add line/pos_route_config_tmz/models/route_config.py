    ...
    def _get_name(self): 
        for doc in self:
            if doc.fecha_filtro:
                ...
                rec = self.env['pos_route_config_tmz.route_config'].search([('create_date', '>=', date_start_search), ('create_date', '<=', date_end_search), ("company_id.name", "=", doc.company_id.name)]) #search([('anno', '=', doc.fecha_filtro)]) 
...
class reporte_s(models.Model): #este almacena los datos
    ...
    departure_date = fields.Datetime(string='Fecha de creación', default=fields.Datetime.now()) #Fecha de salida con H,M,S
    name = fields.Char(string='Referencia de ruta', default='Nuevo')
    #fecha_filtro = fields.Date(string='Fecha de configuración de rutaq: ', default=fields.Date.today())#NO
    #fecha_reporte = fields.Date(string='Fecha  reporte: ', default=fields.Date.today())#NO

    #route_config_id = fields.Many2one(string='Route configuration', comodel_name='pos_route_config_tmz.route_config')#NO
