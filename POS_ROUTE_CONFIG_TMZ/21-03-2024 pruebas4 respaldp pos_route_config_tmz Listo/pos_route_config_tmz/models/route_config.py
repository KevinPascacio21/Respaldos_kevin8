...
class RC_report(models.Model): 
    ...
    def write(self, vals):
        vals['name'] = 'Reporte de rutas de configuración: ' + vals['fecha_filtro']
        if 'fecha_filtro' in vals:
        # Seleccionar los registros actualizados
            records = self.filtered(lambda r: r.id in vals.keys())
        for record in records:
            #Restar 7 hrs a la fecha de creacion de la ruta route_creation_date para que aparezca la hora correcta en el reporte xlsx igual que en la vista y en PTV Rutas
            route_creation_datetime = fields.Datetime.from_string(record.route_creation_date)# Convertir route_creation_date a un objeto datetime
            new_datetime = route_creation_datetime - timedelta(hours=7)# Restar 7 horas
            vals['route_creation_date'] = new_datetime.strftime('%Y-%m-%d %H:%M:%S')# Actualizar el valor en vals

            #Restar 7 hrs a la fecha de salida
            departure_datetime = fields.Datetime.from_string(record.departure_date)# Convertir departure_date a un objeto datetime
            new_departure_datetime = departure_datetime - timedelta(hours=7)# Restar 7 horas
            vals['departure_date'] = new_departure_datetime.strftime('%Y-%m-%d %H:%M:%S')# Actualizar el valor en vals

            #Restar 7 hrs a la fecha de llegada
            arrival_datetime = fields.Datetime.from_string(record.arrival_date)# Convertir arrival_date a un objeto datetime
            new_arrival_datetime = arrival_datetime - timedelta(hours=7)# Restar 7 horas
            vals['arrival_date'] = new_arrival_datetime.strftime('%Y-%m-%d %H:%M:%S')# Actualizar el valor en vals
