...
class RouteConfigReport(models.Model):
    _name = 'RC_report.pos_route_config_RC_report' 
    _inherit = 'RC_report.report_xlsx.abstract' #---------------------------------
    _description = 'Reporte de configuración de rutas'
    
    def generate_xlsx_report(self, workbook, data, catalogo_de_variables):
        for ctrl in catalogo_de_variables:
            report_name = 'Reporte'
            ...
            sheet.write(2, 20, 'Litros a cobrar', title_cell)
            ...
            for line in ctrl.relacion_ids:
                sheet.write(row, 2, line.arrival_date, format_cell)
                sheet.write(row, 3, line.route_config_id, format_cell)
                ...
                sheet.write(row, 9, line.en_weight, format_cell)
