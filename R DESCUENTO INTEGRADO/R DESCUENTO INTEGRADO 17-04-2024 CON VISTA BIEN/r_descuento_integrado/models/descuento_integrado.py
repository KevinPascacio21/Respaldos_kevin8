    ...
    def _get_name(self): 
        for doc in self:
            if doc.fecha_inicio and doc.fecha_final:
                ...
                # rec = self.env['account.move'].search([('invoice_date', '>=', date_start_search), ('invoice_date', '<=', date_end_search), ("company_id.name", "=", doc.company_id.name), ("total_cash_discount", ">", 0), "|", ("edi_state","=","sent"), ("edi_state","=","to_cancel"), ('move_type', '=', 'out_') ])
                notas = self.env['account.move'].search([('invoice_date', '>=',  doc.fecha_inicio), ('invoice_date', '<=', doc.fecha_final)])#, ("edi_state","=","sent"), ('move_type', '=', 'out_invoice'), ("edi_state","=","to_cancel") ]) #("total_cash_discount", ">", 0)
                # notas = self.env['account.move'].search([("invoice_date","=",date_start_search), ('state', '=', 'posted'), ('move_type', '=', 'out_refund'), ('edi_state','=','sent'), ('payment_state', '=', 'paid')])
                
                
                                # notas = self.env['account.move'].search([('invoice_date', '>=',  doc.fecha_inicio), ('invoice_date', '<=', doc.fecha_final), ("total_cash_discount", ">", 0), ("edi_state","=","sent"), ('move_type', '=', 'out_invoice'), ("edi_state","=","to_cancel") ])
